...
Priority correction QA-003: qa_native owns agent/qa-native-seatfix in native-seatfix worktree, based76a9b64; freeze original native branch. Repeated unsolicited accepted input in three native runs blocks controlled evidence. Scope only private-Xserver physical input gating for automated/adaptive runs, preserving XTEST and manual opt-in, exact device ownership/cleanup and repeated proof. bootstrap_m0 independently audits/rereviews; do not alter host devices or count contaminated runs. M0 complete source/evidence tipb647a1e awaits independent review; its reviewer qa_native resumes after this correction.

Current dependency work: direct XI2 physical-slave disable caused an evidenced Xwayland24.1.13 crash, so core_m2 owns separate inputless Weston feasibility probe in agent/native-backend-probe (`tools/qa_backend_probe`, `evidence/backend-probe` only). qa_native retains harness integration/QA-004 watchdog; no duplicate writes. M0 independent review transferred to core_m2, partial report2dbdd1a; native acceptance still pending. M2 source68ce558 is frozen,80 guarded logical checks pass independently, but native/independent core acceptance pending.
