...
Preview controls: WASD/mouse, Shift faster, Escape releases capture, click recaptures; F toggles an inspection flashlight;1–8 select the unchanged fixed review camera poses (entry, wide room, vent, damp carpet, column, fixture, cove corner, close paper).9 adds genuinely oblique paper inspection and0 adds a lateral displaced-board view. P saves an original-resolution PNG plus frame/camera/renderer/measurement metadata; wait for its `M4_CAPTURE ... result=0` line before another capture or stopping. L cycles all fixture states.

The viewer disables mouse accumulation and gates look/movement for two process frames on capture. Wait for the `M4_CAPTURE_GATE` transition `capture_ready` before dispatching a controlled movement request. Event traces retain exact native deltas and whether look was applied or discarded during settling; the guard does not conceal arbitrary large deltas afterward. This is a scoped diagnostic-camera correction, not the production controller/InputGate. Logical checks can run with `--headless --path tools/art/environment/preview --script res://verify_preview_diagnostics.gd`; they cannot establish actual native recapture behavior.

Reversible diagnostic controls: F2 switches2x MSAA/off; F3 cycles100/75/50% render scale; F4 replaces architecture shaders with one uniform rough StandardMaterial while retaining all geometry; F5 switches direct fixture lights without changing emitter materials; F6 toggles glow; F7/F8 toggle SSIL/SSAO; F9 switches the original four fixture shadows off/back on; F10 toggles TAA. F4 is a combined material/batching diagnostic, not proof of pure shader cost: compare actual draw calls. None is a production quality preset. Defaults preserve the authored room. Metadata records exact active render options, flashlight, fixture states and absent/present fog/GI/probes. Each change/pose resets the measurement window; samples begin after3 seconds. CPU rendering setup and viewport GPU cost are measured separately from frame interval. These conveniences never count as independent experiential campaign play. The production project uses its own controls.
