...
Host: Fedora 44, Xwayland 24.1.13, Python 3.14/PythonXlib 0.33, FFmpeg 8.1.2 with x11grab/Pulse/libx264/FLAC, PipeWire 1.6.8, Godot 4.7.2.stable.official.ed1daf0bf, Mesa 26.1.8 RADV, integrated Radeon 660M. NumPy 2.4.6 supports media/freshness analysis; Pillow supports PNG encoding and pixel comparisons. The corrected controlled backend additionally requires Weston 15.0.1 and XComposite 0.2 or newer. Exact run results and limitations are recorded in `evidence/native-seatfix/`, not inferred from these requirements.
...
Before the session becomes ready, the runner calls XComposite automatic redirection on **only the verified owned game window**. It never redirects the root or a root-wide subwindow hierarchy. A requested PNG names a new reference to that window's current off-screen backing pixmap, reads its full native pixels, and frees the reference. This avoids keeping an obsolete pixmap after remap/resize. Geometry, ownership, TrueColor masks and the supported 32-bit little-endian pixel layout are checked; unsupported layouts fail closed. X requests share the input-safety lock; CPU conversion/PNG encoding runs outside it. Sidecars identify the actual acquisition method, target, time and X readback duration. Cleanup explicitly unredirects before closing X; the server also releases redirection when the owning client dies. See the primary [XComposite reference](https://xorg.freedesktop.org/archive/X11R7.5/doc/man/man3/Xcomposite.3.html).

Recording starts asynchronously, finishes at its requested duration, and receives an FFprobe/hash sidecar. Omit `record-stop` when allowing it to finish. FFmpeg captures the Composite-redirected **window ID**, never the host root window. Its readback remains x11grab, separately identified from PNG named-pixmap readback. It records only `<unique_sink>.monitor`, encodes libx264 ultrafast/CRF18/yuv420p and FLAC, and writes a Matroska container. Geometry/unmap/ownership changes are failures. A 60 fps encoded stream can duplicate images; application frame telemetry alone cannot establish capture freshness. Validate changed actual images and pulse timing, including after recording stops.
...
`prove_freshness.py` runs without an active recorder, preferably immediately after the ten-second media proof. Seven changed captured/menu/pause states each acquire the external screenshot **before** pressing F12 to request the fixture's actual `frame_post_draw` viewport readback. Every full-resolution RGB pixel must match the later settled reference; the HUD is included, expected state is correlated, and successive requested states must change the image. No animated barcode or forced continuous redraw is added to mask a presentation defect. This fixture-only exact comparison is not a universal frame-lock guarantee for moving production scenes. Production captures still need current viewport/state or temporal corroboration. An identical old image despite changed state is a failure, even if all input/safety checks pass.

`prove_safety.py` runs after those checks. It compares a quiet interval, verifies compositor/XI2 policy, measures real application key releases during fault conditions, tests private-root blur, and **ends the run with SIGTERM to the verified owned supervisor during a ten-second hold**. It then checks cleanup and raw-source provenance. Run `native.py stop` yourself if not using that deliberate terminal test. Offline `python3 -m unittest discover -s tools/qa/tests -v` uses fake X/image fixtures and never counts as native proof.
...
Correction development-01 proved that XI2 disabling detaches Xwayland devices into FloatingSlave state. Development-02 then disabled all four private proxies successfully but Xwayland 24.1.13 crashed on a native callback when the game mapped. That attempt is a retained failure, not a supported mode. The final approach does not disable devices: input is isolated at the compositor boundary. Separate headless feasibility probes also exposed intermittent stale X11 captures despite 60 FPS application telemetry. At exact clean source `0076de5`, proof-02's `06-lease-during-capture.png` is byte-identical to older `04-final.png` despite changed actual position. That capture proof is explicitly **FAIL (QA-005)**, regardless of its passing input/media/safety counters. The window-local Composite correction and new freshness challenge require fresh exact-commit evidence and separate independent acceptance.
