...
The initial `state_changed(..., LOADING, ...)` notification precedes synchronous storage and has no issued load token (`pending_generation == -1`). Neither completion nor failure accepts that sentinel, zero, negative, stale, or unissued generations. The coordinator obtains its positive token from `load_started`, after clock restoration, and resolves it once.
...
Barrier completion subscribers may synchronously begin a newer generation. An `arrive` return value still belongs to its originating generation, independent of any reentrant retry or subscriber mutation. `outcome()` instead describes the barrier's current generation, which may already have changed after a callback. Caller and signal receipts are separate detached values.
